...
from model.DCFormer_Depth import Model as Model_dupl
...
def step(split, opt, actions, dataLoader, model, optimizer=None, epoch=None):
    ...
    for i, data in enumerate(tqdm(dataLoader, 0, ncols=70)):
        ...
        if opt.test_augmentation and split =='test':
            input_2D, output_Z = input_augmentation(input_2D, model_trans, joints_left, joints_right)
        else:
            input_2D = input_2D.view(N, -1, opt.n_joints, opt.in_channels, 1).permute(0, 3, 1, 2, 4).type(torch.cuda.FloatTensor)
            output_Z = model_trans(input_2D, is_3dhp=True)[:, :, :, -1].unsqueeze(-1)

        output_Z = output_Z * scale.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).repeat(1, output_Z.size(1),opt.out_joints, 1) #
        output_Z_single = output_Z[:,opt.pad].unsqueeze(1) 
        out_target = out_target[:, :, :, -1].unsqueeze(-1)

        if split == 'train':
            # out_target = out_target[:, opt.pad].unsqueeze(1) 
            pred_out = output_Z
        elif split == 'test':
           pred_out = output_Z_single
...
def input_augmentation(input_2D, model_trans, joints_left, joints_right):
    ...
    output_Z_flip = model_trans(input_2D_flip, is_3dhp=True)
    #output_Z_flip[:,:,:, 0] *= -1
    output_Z_flip[:, :, joints_left + joints_right] = output_Z_flip[:, :, joints_right + joints_left]
    output_Z_non_flip = model_trans(input_2D_non_flip, is_3dhp=True)
...
if __name__ == '__main__':
    ...
    model = {}
    model['trans'] = nn.DataParallel(Model_dupl(opt,2,1)).cuda()
    ...
    for epoch in range(1, opt.nepoch):
        ...
        if opt.test == 1:
            ...
            if opt.train and data_threshold < opt.previous_best_threshold:
                opt.previous_name = save_model(opt.previous_name, opt.checkpoint, epoch, data_threshold, model['trans'], '3dhp_depth')
                if opt.refine:
                    opt.previous_refine_name = save_model(opt.previous_refine_name, opt.checkpoint, epoch,
                                                              data_threshold, model['refine'], 'refine_3dhp_depth')
