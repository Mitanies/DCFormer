...
def get_pose3D(video_path, output_dir):
    ...
    model_path_pre = args.previous_dir_scale
    pre_dict_pre = torch.load(model_path_pre)
    model_path_trans = args.previous_dir_depth
    pre_dict_trans = torch.load(model_path_trans)

    ## Reload 
    #model = Model(args).cuda()
    model_pre = nn.DataParallel(Model_scale(args,2,pre_dict_pre['module.regress_head.weight'].shape[0])).cuda()
    model_trans = nn.DataParallel(Model_depth(args,2,pre_dict['module.regress_head.weight'].shape[0])).cuda()
    ...
    model_pre.load_state_dict(model_dict_pre)
    model_pre.eval()

    model_dict_trans = model_trans.state_dict()
    for name, key in model_dict_trans.items():
        model_dict_trans[name] = pre_dict_trans[name]
    model_trans.load_state_dict(model_dict_trans)
    model_trans.eval()

    model_dict_agg = model_agg.state_dict()
    model_path_agg = args.previous_dir_agg
    pre_dict_agg = torch.load(model_path_agg)
    for name, key in model_dict_agg.items():
        model_dict_agg[name] = pre_dict_agg[name]
    model_agg.load_state_dict(model_dict_agg)
    model_agg.eval()
    ...
    for i in tqdm(range(video_length)):
        ...
        output_3D_z_non_flip = model_trans(input_2D_non_flip)[:,:,:,-1].unsqueeze(-1)
        output_3D_xy_non_flip = model_pre(input_2D_non_flip)[:,:,:,0:2]
        output_3D_z_flip         = model_trans(input_2D_flip)[:,:,:,-1].unsqueeze(-1)
        output_3D_xy_flip         = model_pre(input_2D_flip)[:,:,:,0:2]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--previous_dir_scale',type=str, default=''./checkpoint/', help='checkpoint')
    parser.add_argument('--previous_dir_depth',type=str, default=''./checkpoint/', help='checkpoint')
    parser.add_argument('--previous_dir_agg',type=str, default=''./checkpoint/', help='checkpoint')
