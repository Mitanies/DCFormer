...
class Model(nn.Module):
    def __init__(self, args, in_dim=2, out_dim=3, layers=None):
        super().__init__()
        ...
        #self.batch_norm = nn.BatchNorm2d(in_dim, eps=1e-05, momentum=0.1,, affine=True)
    ...
    def forward(self, x, pre_mask=False, mask=None, spatial_mask=None, is_3dhp=False):
        ...
        if pre_mask:
            ...
            x[:,spatial_mask] = self.spatial_mask_token.expand(b,f*self.spatial_mask_num,2)
            #print(spatial_mask.shape)
            #print(x[spatial_mask].shape, self.spatial_mask_token.expand(b,f*self.spatial_mask_num,2).shape) 

            #x_vis = x[:,~mask].reshape(b, -1, s, c)  # ~mask means visible
            #expand_pos_embed = self.dec_pos_embedding.expand(b, -1, -1, -1).clone()
            #pos_emd_vis = expand_pos_embed[:, ~mask].reshape(b, -1, s, c)
            #pos_emd_mask = expand_pos_embed[:, mask].reshape(b, -1, s, c)
            #x = torch.cat([x_vis + pos_emd_vis, self.mask_token + pos_emd_mask], dim=1)        
...
class STC_ATTENTION(nn.Module):
    ...
    def forward(self, input):
        ...
        x_s = att_s @ v_s + sep2_s + 1e-6 * self.drop(sep_s)  # b*h*t,s,c//2//h ##
...
class STC_BLOCK(nn.Module):
    def __init__(self, d_time, d_joint, d_coor):
        ...
        self.stc_att2 = STC_ATTENTION(d_time, d_joint, d_coor) ###
