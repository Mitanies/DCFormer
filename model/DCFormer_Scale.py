...
class Model(nn.Module):
    def __init__(self, args, in_dim=2, out_dim=3, layers=None):
        ...
        d_hid, frames = args.d_hid, args.frames
    ...
    def forward(self, x, pre_mask=False, mask=None, spatial_mask=None, is_3dhp=False):
        ...
        if is_3dhp:
            x = x[:, :, :, :, 0].permute(0, 2, 3, 1).contiguous()  # B,T,J,2,1 (for 3dhp) #T=frame+2
...
class STC_ATTENTION(nn.Module):
    ...
    def forward(self, input):
        ...
        x_s = att_s @ v_s + sep2_s + 1e-4 * self.drop(sep_s)  # b*h*t,s,c//2//h
